...
class ClotheServiceInline(admin.TabularInline):
    model = ClotheService
    fields = ['clothe','qty', 'price','total_items_']
    readonly_fields = ['total_items_', 'price']
...
class MakeupServiceInline(admin.StackedInline):
    model =  MakeupService
    fields = ['package','note','price','total_items_']
...
class AccessoryServiceInline(admin.TabularInline):
    ...
    fields = ['product','qty', 'price','total_items_']
    #raw_id_fields = ['product']
...
def total_row(obj):
    total = total_cart_raw(obj) + total_incurred_raw(obj)
    return total
...
class CartAdmin(admin.ModelAdmin):
    model= Cart 
    list_display=('id','user','client','created_at','wedding_date', 'total_cart', 'total_incurred', 'total', 'paid_','receivable_')
    fields = ['user','client','wedding_date','note','total_cart', 'total_incurred', 'total','paid_', 'receivable_']
    list_display_links=('client',)
    search_fields=('client',)
     #['created_at','wedding_date',]
    readonly_fields = [ 'created_at','total_cart', 'total_incurred', 'total', 'paid_','receivable_']
    ...
    def get_form(self, request, *args, **kwargs):
         form = super(CartAdmin, self).get_form(request, *args, **kwargs)
         form.deposit = 0
